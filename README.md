...
Для обработки заявок на бронирование используется фоновый сервис (основанный на `BackgroundService` и `Channel`). 
При вызове `POST /api/events/{id}/book` заявка сохраняется в памяти со статусом `Pending` и помещается в неблокирующую очередь.
Фоновый процесс асинхронно читает заявки из очереди, имитирует ресурсоемкую обработку (с помощью задержки) и случайным образом меняет статус заявки на `Confirmed` или `Rejected`. Данный подход не блокирует HTTP-поток и обеспечивает высокую отзывчивость API.
...
4. **Завершение:** Через некоторое время фоновый сервис завершает обработку, и статус бронирования меняется на `Confirmed` (успех) или `Rejected` (отказ).
